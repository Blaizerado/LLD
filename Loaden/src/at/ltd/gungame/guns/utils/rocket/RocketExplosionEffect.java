...
public class RocketExplosionEffect {

	public static void makeEffect(Location loc, Double radius, GunInterface gi) {

		List<Block> blocks = getNearbyBlocks(loc, (int) Math.round(radius));
		for (Block b : blocks) {
			if (b.getType() == Material.BARRIER) {
				continue;
			}
			for (Player p : getPlayersNearLocation(loc, 50)) {
		...
		Particles.ROCKET_Explosion(loc);
		loc.getWorld().playSound(loc, Sound.ENTITY_GENERIC_EXPLODE, gi.getVolume(), 10);
