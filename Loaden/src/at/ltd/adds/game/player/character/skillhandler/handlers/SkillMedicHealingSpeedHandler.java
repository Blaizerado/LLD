...
public class SkillMedicHealingSpeedHandler implements SkillHandlerInterface, Listener {
	...
	public void on(EventPlayerHealing e) {
		if (e.isCanceled()) {
			int time = Healing.MIN_LAST_HEAL_TIME;
			SkillData sd = SkillData.getData(e.getPlayer());
			float lvl = sd.getLevel(SkillGroupNames.MEDIC_REGENERATION_SPEED.name());
			if (lvl == 0) {
				return;
			}
			float maxlvl = SkillGroupManager.getGroup(SkillGroupNames.MEDIC_REGENERATION_SPEED.name()).getLevels();
			int removetime = time / 3;
			removetime = (int) (removetime * ((lvl / maxlvl)));
			time -= removetime;
			if (e.getLastHealing() < time) {
				e.setCanceled(false);
			}
		}
