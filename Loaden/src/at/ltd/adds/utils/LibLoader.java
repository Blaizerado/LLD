...
public class LibLoader {
	...
	@ConfigAble(key = "", value = "/home/SERVER_EWI/ltd-6/server/bukkit/lib")
	public static String libfolder;

	public static void init() {
		Config.loadMyClass("LibLoader", LibLoader.class);
		for (String libname : new File(libfolder).list()) {
			File lib = new File(libname);
			if (lib.getName().contains(".jar")) {
				System.out.println("LOADING LIB " + lib.getName());
				try {
	...
	public static JarFile getRunningJar() throws IOException {
		...
		String path = new File(LibLoader.class.getProtectionDomain().getCodeSource().getLocation().getPath())
				.getAbsolutePath();
